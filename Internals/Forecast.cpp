...
double Forecast::factorForecaster(const Eigen::MatrixXd& X, const Filtering::KFS_fit& Fit, const int& K, const int& frequency, const int& delay, const int& h, const int& x0_ind)
{
    ...
    if (frequency == 4)
    {
        ...
        F_hat(all, seq(Fit.order + 1, last)) = Fit.F(seq(0, K - 1), seq(0, last - 1));
        ...
        else
        {
            while (t <= h + 2)
            {
                ...
                F_temp(all, tt) = 1. / 3. * (F_hat(all, t) + 2. * F_hat(all, t - 1) + 3. * F_hat(all, t - 2) + 2. * F_hat(all, t - 3) + F_hat(all, t - 4));
        ...
        {
            DataHandle::removeCol(F_temp, 0, 0);
            DataHandle::removeElement(x0_temp, 0, 0);
            --tau;
        }
    ...
    else if (frequency == 12)
    {
        ...
        /* Case 2: equal frequency */

        // Clean up the data and the factors
        F_temp = Eigen::MatrixXd::Zero(K, h + 1);
        x0_temp = X(seq(0, h - 1), x0_ind);
        F_temp(all, seq(Fit.order - 1, last)) = Fit.F(seq(0, K - 1), seq(0, last - 1));
        x0_temp = X(seq(0, h), x0_ind);

        // Remove the first o + 1 observations since these observations are lost in the estimation step.
        // Accordingly, decrease the index corresponding to the last observation for which the VOI is observed at forecasting date (tau) and decrease the index of the realisation of the forecast (last_obs).
        tau = h - delay;
        for (int o = 1; o < Fit.order; ++o)
        {
            DataHandle::removeCol(F_temp, 0, 0);
            DataHandle::removeElement(x0_temp, 0, 0);
            --tau;
    ...
    lambda_0 = ((F_temp(all, seq(0, tau)) * F_temp(all, seq(0, tau)).transpose()).llt().solve(Eigen::MatrixXd::Identity(K, K)) * (F_temp(all, seq(0, tau)) * x0_temp));
