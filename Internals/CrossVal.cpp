...
)
{
    ...
    // Vectors
    Eigen::VectorXi freq_ind(months), fc_dates(H);
    ...
    Eigen::MatrixXi Check_dates = Eigen::MatrixXi(int(floor(double(cv_dates) / 2.)), 2);
    ...
    Eigen::MatrixXd fes(fc_dates.size(), 2), all_l2 = Eigen::VectorXd::Constant(max_models, 1e-6);
    Eigen::VectorXd res_sq(fc_dates.size());
    Eigen::MatrixXi all_sel = Eigen::MatrixXi::Constant(R, max_models, months);
    Eigen::VectorXi curr_sel_local(R);
...
)
{
    ...
    // Vectors
    Eigen::VectorXd lambda_0(R);
    Eigen::VectorXi ind_vec(R), freq_ind(months), curr_sel(R);
    ...
    for (int i = 0; i < max_models; ++i) {
        ...
        if (decorr_errors)
        {
            X(all, freq_ind) = X(all, freq_ind) * DFM.C.transpose();
        }
...
)
{
    ...
    // Vectors
    Eigen::VectorXd lambda_0(R);
    Eigen::VectorXi freq_ind(months);
...
#pragma omp parallel for schedule(dynamic) shared(results)
    for (int i = 0; i < max_models; ++i) {
        ...
        if (decorr_errors)
        {
            X_local(all, freq_ind) = X_local(all, freq_ind) * DFM.C.transpose();
...
)
{
    ...
    // Vectors
    Eigen::VectorXd lambda_0 = Eigen::VectorXd::Zero(R), l1 = Eigen::VectorXd::Zero(R), all_l2 = Eigen::VectorXd::Constant(max_models, 1e-6);
    ...
    if (l1_min_nan || l1_max_nan) {
        ...
        // Vectors
        Eigen::VectorXd y_temp(T), all_l1_thresholds(0);
